...
**Result**: FP8 quantization currently delivers disk savings only (~39%, 5.9G vs 9.6G bf16). Along the way we found and worked around a genuine Model-Optimizer packaging bug (`fp8_per_tensor_gemm.py`'s real-GEMM kernel registration is dead code, so every quantized layer silently fell back to a dequant-to-bf16 path). Even after fixing that, the quantized model is still **~2x slower** than bf16 at `--batch-size 1`, with no real memory win — most likely because there isn't enough matmul work per call at this batch size to amortize FP8 GEMM overhead. Full numbers and root cause in [Results](#results) below.
...
**GPU memory and latency**: measured via a standalone `check_memory.py` (new script, not part of `quantize.py`). It builds a real `VA_Server` the same way `_create_lingbot_va_pipeline()` does, optionally `mto.restore()`s the FP8-compressed `transformer.pt` onto it, then runs one `infer()` denoise chunk while tracking `torch.cuda.max_memory_allocated()` and wall-clock latency. Runs the `baseline`/`quantized` cases as two isolated subprocesses (self-invoking via `--case`) so peak-memory stats from one case never leak into the other. Has hardcoded local paths (`LINGBOT_VA_REPO`, `MODEL_PATH`, `QUANTIZED_CKPT`) — needs editing before reuse elsewhere. Usage: `python check_memory.py`. Config used: `--batch-size 1`, `frame_chunk_size=2`, 256×320.
...
All measurements above were run via plain **PyTorch eager-mode execution** (`VA_Server.infer()` calling the transformer directly) — not TensorRT, and not `torch.compile`. Each of the ~942 quantized layers runs as separate Python-dispatched ops (quantize activation → real FP8 GEMM → dequantize/cast output) with no operator fusion; this per-op dispatch/kernel-launch overhead is a likely factor in the observed slowdown, not yet confirmed by profiling.
